...
| **mkfs.ext4 `-d` populates in `readdir()` order** — same inode-numbering leak as mkfs.ubifs, plus a random UUID per build, wall-clock timestamps in every superblock instance and all inodes, and a randomised `s_hash_seed` (htree) | drop-in replacement builder: empty mke2fs with fixed `-U`, pin `s_hash_seed` BEFORE population, populate via debugfs from a `find \| sort` list (hard links / whitespace names FATAL), `resize2fs -M`, then Python pins all SB + inode timestamps to `$EPOCH`; final `e2fsck -fn` canary fails the build on any structure/checksum breakage | opt/luckfox/mkfs-ext4-deterministic.sh, patch-fs-determinism.sh §7 |
