...
| **KNOWN EXCEPTION, not fixable here:** download.bin's 196 KiB tail is an AES-ECB-encrypted vendor "flashhead" loader (220 repeated 16-byte ciphertext blocks); sign_tool signs its internal head with a random salt per build and the resulting 256 bytes of *ciphertext* (tail+0x600..0x700) differ every build. We cannot re-sign inside it without the vendor's AES key | none — documented only. It affects download.bin/update.img in the bundle (USB-flash packages); the SD card image never contains download.bin (`sd_update.txt` lists env/idblock/uboot/boot/oem/userdata/rootfs), so the shipped `.img` is unaffected | opt/luckfox/deterministic-sign.sh header comment |
