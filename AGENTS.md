...
| **FDT alignment padding in signed FITs (signed builds only):** mkimage appends the signature node to an already-full FDT; when libfdt grows the structure block it copies into a fresh `malloc()`, so the pad bytes after whatever property triggered the grow hold uninitialised heap memory (observed: 3 random bytes after `hashed-nodes` in uboot.img, different every build) | `fitsign.zero_fdt_padding()` zeroes every FDT pad/NOP byte before signing — pads are undefined per spec and no parser reads them; hashed-node pads are already zero at creation so the signed digest is unchanged. Also applied by `canonicalise` for rebuild diffs | opt/luckfox/secure-boot/fitsign.py (`sign_buf`, `cmd_canonicalise`) |
