...
| **Exception to the row above — FIT signature node (signed builds only):** U-Boot 2017.09's mkimage stamps wall-clock time into the `timestamp` property and does NOT honour `SOURCE_DATE_EPOCH` (verified against shipped CI images), and draws its RSA-PSS salt at random; rk_sign_tool is an unpatchable prebuilt binary | after the SDK signs, all four boot-chain images are re-signed with our own tools: digest-derived PSS salt (`shake_256("seedsigner-pss-v1\0" \|\| mhash)`) + zeroed `timestamp` — both outside the signed region, so on-device verification is unaffected (RFC 8017 recovers the salt from the block; random-salt signatures still verify) | opt/luckfox/deterministic-sign.sh via os-build.sh / build-local.sh (`SEEDSIGNER_FIT_SIGNATURE=1` only) |
| **Uninitialised heap in signed FITs (signed builds only):** mkimage's libfdt leaks host heap memory into two places: the FDT alignment padding (when it grows the structure block to append the signature node it copies into a fresh `malloc()` — observed 3 random bytes after `hashed-nodes` in uboot.img) and the memreserve region (a u64 of pointer residue at offset 0x28 of both uboot.img and boot.img, different every build; which bytes happen to collide between two builds varies, so one image pair can look clean while the next desyncs). Neither is inside any signed region | `fitsign.zero_fdt_padding()` zeroes every FDT pad/NOP byte AND the whole memreserve region (header→structure block) before signing — pads are undefined per spec and no FIT boot path reads memreserve; hashed-node pads are already zero at creation so the signed digest is unchanged. Also applied by `canonicalise` for rebuild diffs | opt/luckfox/secure-boot/fitsign.py (`sign_buf`, `cmd_canonicalise`) |
| **initramfs symlink mtimes (signed builds only):** the rootfs-verifier ramdisk's busybox applet links are created with `ln -s` at build time; a bare `touch` dereferences them, so each link kept its wall-clock mtime in the cpio header and desynced boot.img's ramdisk on every build | `touch -h` (operate on the link itself) pins every entry — links included — to `$SOURCE_DATE_EPOCH` before `cpio --reproducible` packs it | os-build.sh / build-local.sh `embed_rootfs_verifier()` cpio step |
| **Flashhead inner signature (signed builds only):** download.bin's 196 KiB tail is an RC4-obfuscated vendor "flashhead" loader — the cipher is RC4 with a hardcoded 16-byte key inside rk_sign_tool, re-initialised per 512-byte chunk; its plaintext is an idblock-style image whose internal head rk_sign_tool signs with a random salt per build, so 256 bytes of *ciphertext* (tail+0x600..0x700) differed every build | after the SDK signs, `rkloader.py sign_buf` decrypts the region (derived from the outer header's second component entry, not hardcoded), re-signs the inner head with the digest-derived salt — PSS verification recovers any salt from the block, so on-device checking is unaffected — re-encrypts and refreshes the LDR trailer; `verify` checks it, `setkey` re-keys its embedded modulus copy, `canonicalise` zeroes it. It affects download.bin/update.img in the bundle (USB-flash packages); the SD card image never contains download.bin (`sd_update.txt` lists env/idblock/uboot/boot/userdata/rootfs), so the shipped `.img` was unaffected | opt/luckfox/secure-boot/rkloader.py (`resign_flashhead`) via deterministic-sign.sh |
...
| **mkfs.ext4 `-d` populates in `readdir()` order** — same inode-numbering leak as mkfs.ubifs, plus a random UUID per build, wall-clock timestamps in every superblock instance and all inodes, and a randomised `s_hash_seed` (htree) | drop-in replacement builder: empty mke2fs with fixed `-U`, pin `s_hash_seed` BEFORE population, populate via debugfs from a `find \| sort` list (whitespace names FATAL; hard links recreated with debugfs `ln` — duplicating them would multiply their blocks and overflow the partition on dev images. This debugfs's `ln` cannot extend directories, so link-receiving dirs are pre-sized with throwaway write+rm pairs; it never bumps i_links_count, fixed in a post-pass; freed dummy slots keep stale mode bits and are zeroed to pristine-free state), then a read-only pass verifies every entry against the source manifest (debugfs -f swallows ENOSPC and still exits 0 — without this a space-starved build ships silently missing files), `resize2fs -M`, Python pins all SB + inode timestamps to `$EPOCH`; final `e2fsck -fn` canary fails the build on any structure/checksum breakage | opt/luckfox/mkfs-ext4-deterministic.sh, patch-fs-determinism.sh §7 |
...
| `pip install fonttools` unpinned: `pyftsubset`'s GPOS output shifts between releases (4.63.0 vs 4.64.0 = JP +532 / KR +538 / SC +550 bytes, glyf/cmap/GSUB byte-identical) — `head`'s checkSumAdjustment then cascades into rootfs.img, update.img and the SD images | `fonttools==4.64.0` pinned at both call sites; venv `pyftsubset` invoked by absolute path with its `fontTools.version` checked against the pin (a `PATH` fallback to a host copy is what made this invisible) | opt/luckfox/compile-translations.sh, opt/build.sh |
| **Reused app checkout carried the previous build's output**: the `seedsigner-repos` volume outlives the build and reuse was gated on HEAD alone, so a repeat build re-slimmed already-slimmed fonts. Re-subsetting is a fixed point (a pruned GPOS cannot be re-expanded), freezing the first slimming version into every later local build; CI clones fresh and never hits it | `restore_pristine()` on every reuse path — `reset --hard` + `clean -fdx`, submodules first (the superproject clean skips dirs containing a `.git`) | opt/luckfox/prepare-app-checkout.sh |

**Maintenance requirements (things that silently break reproducibility):**
* **Bumping `SDK_COMMIT`**: re-verify that `sort-dirents.patch` still applies and every sed/grep target in
...
them.) **The `oem` partition used to be part of the attack surface** — every built `.ko` was packaged to a
separate, unsigned `/oem` partition that the board executed as root. Since 2026-09-23 it is gone: `oem` is
folded into the signed rootfs (`RK_BUILD_APP_TO_OEM_PARTITION=n`, see `apply-partition-layout.sh`), so
`/oem/usr/ko`, the iqfiles and `RkLunch.sh` are covered by the rootfs signature (tier C). A fully fused
board verified the rootfs and then ran a tamper spliced into the old `oem` partition's `RkLunch.sh` as uid 0;
removing the partition removes that path by construction. `assert-kernel-network.sh` (with `REQUIRE_OEM=1`)
still scans the folded payload for stray wireless modules.
...
| **Kernel serial console** | UART2 console stripped from bootargs/DTS (`disable_uart2_console_debug` auto→on **on every variant** — the console shares UART2 with the SEC1210 reader; only explicit false/0 keeps it, for bench debugging without a card reader) | UART2 strip steps in all three builds |
