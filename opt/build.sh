...
build_image() {
  ...
  if [ -n "${DEBUG_ROOTFS}" ] && [ -d "${build_dir}/target" ]; then
    ...
    reproducible_mtime='2025-07-01 00:00:00'
    GZIP='-n' tar --sort=name --mtime="${reproducible_mtime}" --owner=0 --group=0 --numeric-owner \
      --pax-option=exthdr.name=%d/PaxHeaders/%f,delete=atime,delete=ctime \
      -C "${build_dir}" -czf "${rootfs_tar_output}" target
    touch -d "${reproducible_mtime}" "${rootfs_tar_output}"
