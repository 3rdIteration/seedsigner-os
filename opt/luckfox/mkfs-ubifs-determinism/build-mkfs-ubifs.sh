...
# overlayfs vs tmpfs, kernel version), so two builds of an identical /oem tree
# on different machines produced different inode numbers -- and therefore
...
MTD="$SDK_DIR/sysdrv/tools/board/mtd-utils/mtd-utils-2.0.1"
SRC="$MTD/ubifs-utils/mkfs.ubifs"
...
[ -f "$SRC/mkfs.ubifs.c" ] || { echo "build-mkfs-ubifs: SDK source not found at $SRC" >&2; exit 1; }
[ -f "$MTD/include/common.h" ] || { echo "build-mkfs-ubifs: $MTD/include/common.h not found" >&2; exit 1; }
...
# --- 3. copy sources, apply the patch, compile -------------------------------
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
cp -r "$SRC" "$TMP/src"
